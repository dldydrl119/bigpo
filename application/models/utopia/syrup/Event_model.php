...
//        $this->load->library('session');
    ...
    public function eventListData($page = 1)
    {
        ...
        $sql="SELECT @rownum:=@rownum+1 num, idx, name,memo, reg_date from utopia_syrup_event, (SELECT @rownum:= {$offset}) TMP ORDER BY idxs
    ...
    public function eventTotalNum() {
        ...
        $sql = "select count(*) count From utopia_syrup_event ";
...
//        $idxs = $this->db->query($sql)->row()->idxs + 1;
        ...
        $sql="SELECT idxs FROM utopia_syrup_event order by idxs DESC LIMIT 1";
        ...
        $result = $this->db->insert('utopia_syrup_event', $insert_data);
        if($result) {
    ...
    public function eventData($idx) {
        $array = array($idx);
        $sql="SELECT *
            FROM utopia_syrup_event WHERE idx = ?";
    ...
    public function eventEditData($update, $idx) {
        ...
        $result = $this->db->update('utopia_syrup_event', $update);
    ...
    public function eventdel($idx) {
        $array = array($idx);
        $sql= "DELETE FROM utopia_syrup_event WHERE idx = ?";
    ...
    public function eventOrder($idxs, $idx) {
        for($i=0; $i<count($idxs); $i++) {
            $sql= "UPDATE utopia_syrup_event SET idxs = ? WHERE idx = ?";
