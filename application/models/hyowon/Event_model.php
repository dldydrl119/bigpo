...
//        $this->load->library('session');
    ...
    public function eventListData($page = 1)
    {
        ...
        $sql="SELECT @rownum:=@rownum+1 num, idx, name,memo, reg_date from hyowon_event, (SELECT @rownum:= {$offset}) TMP ORDER BY idxs
    ...
    public function eventTotalNum() {
        ...
        $sql = "select count(*) count From hyowon_event ";
...
//        $idxs = $this->db->query($sql)->row()->idxs + 1;
        ...
        $sql="SELECT idxs FROM hyowon_event order by idxs DESC LIMIT 1";
        ...
        $result = $this->db->insert('hyowon_event', $insert_data);
        if($result) {
    ...
    public function eventEditData($update, $idx) {
        ...
        $result = $this->db->update('hyowon_event', $update);
    ...
    public function eventdel($idx) {
        $array = array($idx);
        $sql= "DELETE FROM hyowon_event WHERE idx = ?";
    ...
    public function eventOrder($idxs, $idx) {
        for($i=0; $i<count($idxs); $i++) {
            $sql= "UPDATE hyowon_event SET idxs = ? WHERE idx = ?";
