...
//        $this->load->library('session');
    ...
    public function eventListData($page = 1)
    {
        ...
        $sql="SELECT @rownum:=@rownum+1 num, idx, name,memo, reg_date from kyowon_syrup_event, (SELECT @rownum:= {$offset}) TMP ORDER BY idxs
    ...
    public function eventTotalNum() {
        ...
        $sql = "select count(*) count From kyowon_syrup_event ";
...
//        $idxs = $this->db->query($sql)->row()->idxs + 1;
        ...
        $sql="SELECT idxs FROM kyowon_syrup_event order by idxs DESC LIMIT 1";
        ...
        $result = $this->db->insert('kyowon_syrup_event', $insert_data);
        if($result) {
    ...
    public function eventData($idx) {
        $array = array($idx);
        $sql="SELECT *
            FROM kyowon_syrup_event WHERE idx = ?";
    ...
    public function eventEditData($update, $idx) {
        ...
        $result = $this->db->update('kyowon_syrup_event', $update);
    ...
    public function eventdel($idx) {
        $array = array($idx);
        $sql= "DELETE FROM kyowon_syrup_event WHERE idx = ?";
    ...
    public function eventOrder($idxs, $idx) {
        for($i=0; $i<count($idxs); $i++) {
            $sql= "UPDATE kyowon_syrup_event SET idxs = ? WHERE idx = ?";
