...
//        $this->load->library('session');
    ...
    public function eventListData($page = 1)
    {
        ...
        $sql="SELECT @rownum:=@rownum+1 num, idx, name,memo, reg_date from kyowon_event, (SELECT @rownum:= {$offset}) TMP ORDER BY idxs
    ...
    public function eventTotalNum() {
        ...
        $sql = "select count(*) count From kyowon_event ";
...
//        $idxs = $this->db->query($sql)->row()->idxs + 1;
        ...
        $sql="SELECT idxs FROM kyowon_event order by idxs DESC LIMIT 1";
        ...
        $result = $this->db->insert('kyowon_event', $insert_data);
        if($result) {
    ...
    public function eventEditData($update, $idx) {
        ...
        $result = $this->db->update('kyowon_event', $update);
    ...
    public function eventdel($idx) {
        $array = array($idx);
        $sql= "DELETE FROM kyowon_event WHERE idx = ?";
    ...
    public function eventOrder($idxs, $idx) {
        for($i=0; $i<count($idxs); $i++) {
            $sql= "UPDATE kyowon_event SET idxs = ? WHERE idx = ?";
