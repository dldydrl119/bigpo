...
//        $this->load->library('session');
    ...
    public function eventListData($page = 1)
    {
        ...
        $sql="SELECT @rownum:=@rownum+1 num, idx, name,memo,birth,gender,address, reg_date from rental_sinkpure_event, (SELECT @rownum:= {$offset}) TMP ORDER BY idxs DESC
    ...
    public function eventTotalNum() {
        ...
        $sql = "select count(*) count From rental_sinkpure_event ";
...
//        $idxs = $this->db->query($sql)->row()->idxs + 1;
        ...
        $sql="SELECT idxs FROM rental_sinkpure_event order by idxs DESC LIMIT 1";
        ...
        $result = $this->db->insert('rental_sinkpure_event', $insert_data);
        if($result) {
    ...
    public function eventData($idx) {
        $array = array($idx);
        $sql="SELECT *
            FROM rental_sinkpure_event WHERE idx = ?";
    ...
    public function eventEditData($update, $idx) {
        ...
        $result = $this->db->update('rental_sinkpure_event', $update);
    ...
    public function eventdel($idx) {
        $array = array($idx);
        $sql= "DELETE FROM rental_sinkpure_event WHERE idx = ?";
    ...
    public function eventOrder($idxs, $idx) {
        for($i=0; $i<count($idxs); $i++) {
            $sql= "UPDATE rental_sinkpure_event SET idxs = ? WHERE idx = ?";
