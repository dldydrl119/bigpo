...
class Event_model extends CI_Model
{
    ...
    public function eventListData($page = 1)
    {
        ...
        $sql = "SELECT @rownum:=@rownum+1 num, idx, name,memo, reg_date from utopia_02_sbj_event, (SELECT @rownum:= {$offset}) TMP ORDER BY idxs
    ...
    public function eventTotalNum()
    {
        ...
        $sql = "select count(*) count From utopia_02_sbj_event ";
    ...
    public function eventAddData($insert_data)
    {
        ...
        $sql = "SELECT idxs FROM utopia_02_sbj_event order by idxs DESC LIMIT 1";
        ...
        $result = $this->db->insert('utopia_02_sbj_event', $insert_data);
        if ($result) {
    ...
    public function eventData($idx)
    {
        $array = array($idx);
        $sql = "SELECT *
            FROM utopia_02_sbj_event WHERE idx = ?";
    ...
    public function eventEditData($update, $idx)
    {
        ...
        $result = $this->db->update('utopia_02_sbj_event', $update);
    ...
    {
        $array = array($idx);
        $sql = "DELETE FROM utopia_02_sbj_event WHERE idx = ?";
    ...
    public function eventOrder($idxs, $idx)
    {
        for ($i = 0; $i < count($idxs); $i++) {
            $sql = "UPDATE utopia_02_sbj_event SET idxs = ? WHERE idx = ?";
