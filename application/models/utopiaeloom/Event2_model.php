...
//        $this->load->library('session');
    ...
    public function eventListData($page = 1)
    {
        ...
        $sql="SELECT @rownum:=@rownum+1 num, idx, name,memo,hope,call2,note, reg_date from utopiaeloom_event, (SELECT @rownum:= {$offset}) TMP ORDER BY idxs
    ...
    public function eventTotalNum() {
        ...
        $sql = "select count(*) count From utopiaeloom_event ";
...
//        $idxs = $this->db->query($sql)->row()->idxs + 1;
        ...
        $sql="SELECT idxs FROM utopiaeloom_event order by idxs DESC LIMIT 1";
        ...
        $result = $this->db->insert('utopiaeloom_event', $insert_data);
        if($result) {
    ...
    public function eventData($idx) {
        $array = array($idx);
        $sql="SELECT *
            FROM utopiaeloom_event WHERE idx = ?";
    ...
    public function eventEditData($update, $idx) {
        ...
        $result = $this->db->update('utopiaeloom_event', $update);
    ...
    public function eventdel($idx) {
        $array = array($idx);
        $sql= "DELETE FROM utopiaeloom_event WHERE idx = ?";
    ...
    public function eventOrder($idxs, $idx) {
        for($i=0; $i<count($idxs); $i++) {
            $sql= "UPDATE utopiaeloom_event SET idxs = ? WHERE idx = ?";
