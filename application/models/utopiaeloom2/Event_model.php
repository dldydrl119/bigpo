...
//        $this->load->library('session');
    ...
    public function eventListData($page = 1)
    {
        ...
        $sql="SELECT @rownum:=@rownum+1 num, idx, name,memo,call2,note,marketing_agreement, reg_date from utopiaeloom2_event, (SELECT @rownum:= {$offset}) TMP ORDER BY idx
...
//        $idxs = $this->db->query($sql)->row()->idxs + 1;
        ...
        $sql= "SELECT idxs FROM utopiaeloom2_event INNER JOIN msg_queue USING (idxs) order by idxs DESC LIMIT 1";
