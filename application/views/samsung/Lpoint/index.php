<!DOCTYPE html>
<html lang="ko">

<head>
	<meta charset="utf-8">
	<meta name="viewport" content="width=device-width, initial-scale=1.0" />

	<meta name="keywords" content="Callget">
	<meta name="description" content="Callget">

	<link href="/assets/images/utopia_sbj/favicon.png" rel="shortcut icon" type="image/x-icon">
	<link href="/assets/images/utopia_sbj/favicon.png" rel="icon" type="image/x-icon">

	<title>삼성화재</title>

	<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.3/jquery.min.js"></script>
	<script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.5/js/bootstrap.min.js"></script>

	<link href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.5/css/bootstrap.min.css" rel="stylesheet">

	<link href="/assets/css/sbj_landing.css" rel="stylesheet" type="text/css">

	<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.2.0/jquery.min.js"></script>
	<style>
		.web1 {
			display: none;
		}

		.mobile1 {
			display: block;
		}

		@media (max-width:450px) {
			.web1 {
				display: none;
			}

			.mobile1 {
				display: block;
			}
		}

		#text {
			text-align: center;
			font-weight: bold;
			margin-top: 40px;
		}

		#img {
			margin: 0 0 10px;
			width: 15%;
		}

		#button {
			width: 80%;
		}

		#pop {
			position: fixed;
			z-index: 1;
			top: 0;
			width: 100%;
			height: 100%;
			display: none;
		}

		#pop>article {
			width: 50%;
			position: relative;
			z-index: 2;
			background: white;
			box-shadow: 3px 3px 5px black;
			margin: 12% 0 0 25%;
			cursor: pointer;
			text-align: center;
		}

		.black {
			background: rgba(0, 0, 0, 0.6);

		}

		.info_call {
			width: 100%;
			line-height: 36px;
			color: black;
			border: 1px solid #2970d6;
			font-size: 1.4vw;
		}

		.infoDetail {
			width: 100%;
			padding: 54px 0;
			border: 1px solid #2970d6;
			font-size: 20px;
			line-height: 40px;
		}

		.infobutton {
			width: 100%;
			background-color: #2970d6;
			color: #ffffff;
			border: none;
			line-height: 70px;
			cursor: pointer;
		}

		.infoimg {
			width: 20%;
			border-radius: 100px;
		}

		@media (max-width: 767px) {

			#get_main .fill .middle {
				width: 100%;
				height: 100%;
				position: absolute;
				top: 32%;
			}

			#get_main .manual .manual_area .manual_contents .panel-group .panel-body br {
				display: none;
			}

			#get_main .get_area1 {
				display: inline;
			}

			#get_main .get_area_m {
				display: none;
			}
		}


		/*변경을 일으키는 버튼CSS
                              마우스오버 CSS*/


		.button:hover {
			background-color: #000;
			transition: all linear 1s;
		}
	</style>
	<script>
		$(document).ready(function() {
			$("#list>button:first-of-type").click(function() {
				$("#pop").fadeIn();
				$("#pop").addClass("black");
			});
			$("#pop>article").click(function() {
				$(this).parent().fadeOut();
			});
		});
	</script>

	<script language="JavaScript">
		var ck_click = false;

		function checkIt() {

			

				if (ck_click == true) {
					location = 'tel:080-525-8080';
					// alert('영상.');
					
					return;
				} else {
					alert('영상 시청 후 눌러주세요.');
					javascript: setTimeout('changeColor(0)', 3000);
					ck_click = true;
				}
			}
		
		//
	</script>


</head>

<body>
	<div class="sect_1">
		<div class="container">
			<div class="col-sm-offset-3 col-sm-6 col-xs-12">
				<img src="/assets/images/samsung/Lpoint/main.jpg" alt="메인이미지">
			</div>
		</div>
	</div>

	<div class="sect_2">
		<div class="container">
			<div class="col-sm-offset-3 col-sm-6 col-xs-12">
				<div class="video-bg samsungL_bg_01">
					<div class="video-container">
						<div id="player"></div>
						<iframe width="100%" src="https://www.youtube.com/embed/sbPaYHrlH7E" frameborder="0"></iframe>

					</div>
				</div>
			</div>
		</div>
	</div>

	<div class="get_area1">
		<div class="sect_3 web1">
			<div class="container">
				<div class="col-sm-offset-3 col-sm-6 col-xs-12">
					<div class="get_area1" id="list" style="margin: 20px;">
						<button id="button" class="sbj_01" onclick="location.href='javascript:checkIt()'">
							<p style="color: white; font-size :13px;">365일 24시간 가능</p>상담 예약하기<br>
					</div>
					</button>
				</div>
			</div>
		</div>
	</div>
	<div class="sect_3 mobile1">
		<div class="container">
			<div style="background:#dbd6f4;" class="col-sm-offset-3 col-sm-6 col-xs-12">
				<div style="margin: 20px; ">
					<button id="button" class="samgsun_OCB circle button" onclick="location.href='javascript:checkIt()'">
						<p style="color: white; font-size :13px; margin:0px;">365일 24시간 가능</p>상담 예약하기<br>
					</button>
				</div>
			</div>
		</div>
	</div>
	<!--변경첫째영역-->



	<!-- // 변경을 발생시키는 버튼들 -->
	<!-- <p class="button" onclick="javascript:setTimeout('changeColor(0)',3000);">RED</p>
	<p class="button" onclick="changeColor(1)">PURPLE</p>
	<p class="button" onclick="changeColor(2)">BLUE</p> -->

	<script>
		//  circle1:변경첫째영역 
		// circle1:변경둘째영역 // circle3:변경셋째영역
		// 각각영역을 변수 circle1~3에 담겠다
		var circle1 = document.getElementsByClassName('circle')[0];
		var circle2 = document.getElementsByClassName('circle')[1];
		var circle3 = document.getElementsByClassName('circle')[2];
		//colorarr은 색을 담고있는 배열이다
		//여기서 색을꺼내어 변경영역의 backgroundColor를 변경할것이다
		var colorarr = ['#21409a', 'purple', 'blue']

		//실제로 실행되는 changeColor함수이다
		//매개변수 number를 받아 colorarr배열의 number번째 요소(색)을 부른다
		function changeColor(number) {
			circle1.style.backgroundColor = colorarr[number]
			circle2.style.backgroundColor = colorarr[number]
			circle3.style.backgroundColor = colorarr[number]
		}

		setTimeout(function() {
			console.log('Works!');
		}, 3000);
	</script>

	<div id="pop">
		<article>
			<div class="info_call">상담안내</div>
			<div class="infoDetail">
				<!-- <img class="infoimg" src="/assets/uploads/<?= $data->product_thumb ?>" alt=""> -->
				<div style="height: 30px;"></div>
				<!-- <p><strong>[ <?= $data->product_name ?> ]</strong>를 받으시기 위해 상담예약이 필수 입니다. </p>  -->
				<p>상담을 받으시기 위해 상담예약이 필수 입니다. </p>
				<p><strong>[ 080-525-8080 ]</strong>으로 지금바로 전화주세요.</p><br>
				<p>365일,24시간 상담예약 가능 </p>
			</div>
			<button class="infobutton" type="button">확인</button>
		</article>
	</div>
	<footer>
		<div class="container">
			<div class="col-sm-offset-3 col-sm-6 col-xs-12">
				<img src="/assets/images/samsung/Lpoint/footer.jpg" alt="푸터이미지">
			</div>
		</div>
	</footer>
</body>

</html>