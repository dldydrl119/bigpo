...
class Main extends MY_Controller
{
    ...
    public function loginData() //로그인 데이터
    {
        ...
        if (password_verify($_POST['pass'], $loginData->admin_pass)) {
            $newdata = array(
                'utopia_02_sbj_id'  => $loginData->admin_id,
                'utopia_02_sbj_idx'    => $loginData->admin_idx,
            );
    ...
    {
        $this->session->unset_userdata('utopia_02_sbj_id');

        $this->session->unset_userdata('utopia_02_sbj_idx');
        header("location: " . URL . 'utopia_02/sbj/admin/main/');
        exit;
    }
