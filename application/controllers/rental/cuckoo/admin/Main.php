...
class Main extends MY_Controller
{
    ...
    public function loginData() //로그인 데이터
    {
        ...
        if (password_verify($_POST['pass'], $loginData->admin_pass)) {
            $newdata = array(
                'rental_cuckoo_id'  => $loginData->admin_id,
                'rental_cuckoo_idx'    => $loginData->admin_idx,
            );
    ...
    {
        $this->session->unset_userdata('rental_cuckoo_id');

        $this->session->unset_userdata('rental_cuckoo_idx');
        header("location: " . URL . 'rental/cuckoo/admin/main/');
        exit;
    }
