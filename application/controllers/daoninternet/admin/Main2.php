...
class Main2 extends MY_Controller
{
    ...
    public function loginData() //로그인 데이터
    {
        ...
        if (password_verify($_POST['pass'], $loginData->admin_pass)) {
            $newdata = array(
                'daoninternet_id'  => $loginData->admin_id,
                'daoninternet_idx'    => $loginData->admin_idx,
            );
    ...
    {
        $this->session->unset_userdata('daoninternet_id');

        $this->session->unset_userdata('daoninternet_idx');
        header("location: " . URL . 'daoninternet/admin/main2/');
        exit;
    }
