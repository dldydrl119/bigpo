...
class Event extends MY_Controller
{
    public function __construct()
    {
        ...
        $this->number_func = true;
        $this->gender_func =true;
    }

    public function list($page = 1)
    {
        // $this->daoninternet_nologin();
        $name = "상담신청 관리";

        $col = ['번호', '이름', '연락처', '승인 여부','이용중인 통신사', '변경 희망 통신사', '메모', '등록 시간', '수정'];
...
//            $this->json(array('return'=>false, 'err_msg'=>'데이터 제한수를 넘었습니다.'));
    ...
    public function addData()
    {
        ...
        if ($this->number_func) {
            $insert_data['number'] = $this->input->post('number');
        }
        if ($this->memo_func) {
    ...
    public function editData()
    {
        ...
        if ($this->number_func) {
            $update_data['number'] = $this->input->post('number');
        }
        if ($this->memo_func) {
