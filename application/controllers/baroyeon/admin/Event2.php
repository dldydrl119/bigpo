...
class Event2 extends MY_Controller
{
    public function __construct()
    {
        ...
        $this->memo_func = true;
        $this->age_func = true;
        $this->gender_func = true;
        $this->live_func = true;
        $this->height_func = true;
        $this->education_func = true;
        $this->marriage_func = true;
        $this->hope_func = true;
    ...
    {
        // $this->baroyeon_nologin();
        $name = "상담신청 관리";
        $col = ['번호', '정렬', '이름', '나이', '성별', '키', '학력', '결혼여부', '연락처', '상태', '메모', '마케팅동의', '등록 시간', '수정','삭제'];
...
//            $this->json(array('return'=>false, 'err_msg'=>'데이터 제한수를 넘었습니다.'));
        ...
        $data = [
            ...
            'age_func' => $this->age_func,
            'gender_func' => $this->gender_func,
            'live_func' => $this->live_func,
            'height_func' => $this->height_func,
            'education_func' => $this->education_func,
            'marriage_func' => $this->marriage_func,
    ...
    public function addData()
    {
        ...
        if ($this->live_func) {
            $insert_data['live'] = $this->input->post('live');
        }
        if ($this->height_func) {
            $insert_data['height'] = $this->input->post('height');
        }
        if ($this->education_func) {
            $insert_data['education'] = $this->input->post('education');
        }
        if ($this->marriage_func) {
            $insert_data['marriage'] = $this->input->post('marriage');
        }
        if($this->hope_func) {
    ...
    public function editData()
    {
        ...
        if ($this->age_func) {
            $update_data['age'] = $this->input->post('age');
        }
        if ($this->gender_func) {
            $update_data['gender'] = $this->input->post('gender');
        }
        if ($this->live_func) {
            $update_data['live'] = $this->input->post('live');
        }
        if ($this->height_func) {
        ...
        if ($this->marriage_func) {
            $update_data['marriage'] = $this->input->post('marriage');
        }
