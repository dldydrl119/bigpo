...
class Main2 extends MY_Controller
{
    ...
    public function loginData() //로그인 데이터
    {
        ...
        if (password_verify($_POST['pass'] ,$loginData->admin_pass) ) {
            $newdata = array(
                'baroyeon_id'  => $loginData->admin_id,
                'baroyeon_idx'	=> $loginData->admin_idx,
            );
...
//        $this->session->sess_destroy();
        $this->session->unset_userdata('baroyeon_id');

        $this->session->unset_userdata('baroyeon_idx');
        header("location: ".URL.'baroyeon/admin/main2/');
        exit;
    }
