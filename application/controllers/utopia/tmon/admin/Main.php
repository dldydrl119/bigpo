...
class Main extends MY_Controller
{
    ...
    public function loginData() //로그인 데이터
    {
        ...
        if (password_verify($_POST['pass'], $loginData->admin_pass)) {
            $newdata = array(
                'utopia_tmon_id'  => $loginData->admin_id,
                'utopia_tmon_idx'    => $loginData->admin_idx,
            );
    ...
    {
        $this->session->unset_userdata('utopia_tmon_id');

        $this->session->unset_userdata('utopia_tmon_idx');
        header("location: " . URL . 'utopia/tmon/admin/main/');
        exit;
    }
