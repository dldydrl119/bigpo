...
class Main extends MY_Controller
{
    ...
    public function loginData() //로그인 데이터
    {
        ...
        if (password_verify($_POST['pass'] ,$loginData->admin_pass) ) {
            $newdata = array(
                'kyowon_id'  => $loginData->admin_id,
                'kyowon_idx'	=> $loginData->admin_idx,
            );
...
//        $this->session->sess_destroy();
        $this->session->unset_userdata('kyowon_id');

        $this->session->unset_userdata('kyowon_idx');
        header("location: ".URL.'kyowon/admin/main/');
        exit;
    }
