...
class Main extends MY_Controller
{
    ...
    public function loginData() //로그인 데이터
    {
        ...
        if (password_verify($_POST['pass'] ,$loginData->admin_pass) ) {
            $newdata = array(
                'kyowon_syrup_id'  => $loginData->admin_id,
                'kyowon_syrup_idx'	=> $loginData->admin_idx,
            );
    ...
    public function logout(){
        $this->session->unset_userdata('kyowon_syrup_id');

        $this->session->unset_userdata('kyowon_syrup_idx');
        header("location: ".URL.'kyowon/syrup/admin/main/');
        exit;
    }
