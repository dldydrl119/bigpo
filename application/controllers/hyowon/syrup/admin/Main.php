...
class Main extends MY_Controller
{
    ...
    public function loginData() //로그인 데이터
    {
        ...
        if (password_verify($_POST['pass'] ,$loginData->admin_pass) ) {
            $newdata = array(
                'hyowon_syrup_id'  => $loginData->admin_id,
                'hyowon_syrup_idx'	=> $loginData->admin_idx,
            );
    ...
    public function logout(){
        $this->session->unset_userdata('hyowon_syrup_id');

        $this->session->unset_userdata('hyowon_syrup_idx');
        header("location: ".URL.'hyowon/syrup/admin/main/');
        exit;
    }
