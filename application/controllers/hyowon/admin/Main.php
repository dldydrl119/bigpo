...
class Main extends MY_Controller
{
    ...
    public function loginData() //로그인 데이터
    {
        ...
        if (password_verify($_POST['pass'] ,$loginData->admin_pass) ) {
            $newdata = array(
                'hyowon_id'  => $loginData->admin_id,
                'hyowon_idx'	=> $loginData->admin_idx,
            );
...
//        $this->session->sess_destroy();
        $this->session->unset_userdata('hyowon_id');

        $this->session->unset_userdata('hyowon_idx');
        header("location: ".URL.'hyowon/admin/main/');
        exit;
    }
