$(document).ready(function () {
  ...
  $("body").on("change", "#slideimage, #thumbnail_image", function () {
    ...
    $(this)
      .next()
      .next()
      .attr("src", "/data/image/" + json.url[0]);
    reg_files = $(this).val();
  });
        ...
        for (var variable in json.url) {
          if (json.url.hasOwnProperty(variable)) {
            if ($("textarea+iframe").length) {
              ...
              $img
                .removeClass("req def")
                .attr("src", json.path + json.url[variable]);
              $img
                .appendTo(common.target.parent())
                .after(
                  '<input type="hidden" name="' +
                    $img.attr("id").replace(/[0-9]/g, "") +
                    '[]" value="' +
                    json.url[variable] +
                    '">'
                );
  ...
  $("body").on("click", "#logingo", function () {
    ...
    if (req) {
      ...
      var json = jsonreturn("/daoninternet/admin/main/loginData", data);
      if (json.return == true) {
        location.href = "/daoninternet/admin/event/list";
      } else {
  ...
  $("body").on("click", "#logingo2", function () {
    var req = true;
    $(".req").each(function () {
      if (!$(this).val()) {
        req = false;
    ...
    if (req) {
      ...
      var json = jsonreturn("/daoninternet/admin/main/loginData", data);
      if (json.return == true) {
        location.href = "/daoninternet/admin/event2/list";
      } else {
        alert("아이디 비밀번호가 일치하지 않습니다");
      }
    } else {
  ...
  function admin() {
      ...
      $("body").on("click", "#eventgo, #eventedit", function () {
        ...
        if (check) {
          ...
          if ($(this).attr("id") == "eventgo") {
            var json = jsonreturn("/daoninternet/admin/event/addData", data);
            if (json.return == true) {
              alert("등록되었습니다");
              location.href = "/daoninternet/admin/event/list";
            }
          } else {
            var json = jsonreturn("/daoninternet/admin/event/editData", data);
            if (json.return == true) {
              alert("수정되었습니다");
              // location.href = "#";
            }
    ...
    this.event2 = function () {
      $("body").on("click", "#eventgo2, #eventedit2", function () {
        var check = true;
        if (check) {
          var form = $("form")[0];
          var data = new FormData(form);
          if ($(this).attr("id") == "eventgo") {
            var json = jsonreturn("/daoninternet/admin/event2/addData", data);
            if (json.return == true) {
              alert("등록되었습니다");
              location.href = "/daoninternet/admin/event2/list";
            }
          } else {
            var json = jsonreturn("/daoninternet/admin/event2/editData", data);
            if (json.return == true) {
              alert("수정되었습니다");
              // location.href = "#";
            }
    ...
    $("body").on("click", ".btn-show", function () {
      var result = json($(this).attr("data-url"), {
        ...
        idx: $(this).parent().parent().attr("data-idx"),
      });
      location.reload();
    });
    //
